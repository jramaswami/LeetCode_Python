...
class Solution():
    def findLadders(self, begin_word, end_word, word_list):
        ...
        while queue:
            ...
            for i, _ in enumerate(wd):
                for c in string.ascii_lowercase:
                    wd0 = wd[:i] + c + wd[i+1:]
                    if wd0 in words and distance[wd] + 1 <= distance[wd0]:
                        parents[wd0].add(wd)
        ...
        def dfs(wd, path):
            ...
            if wd == begin_word:
                yield path[::-1]
            for wd0 in parents[wd]:
                yield from dfs(wd0, path)
            path.pop()

        return list(dfs(end_word, []))
...
def test_7():
    ...
    expected = [["aaaaa","aaaaz","aaawz","aavwz","avvwz","vvvwz","vvvww","wvvww","wwvww","wwwww","ywwww","yywww","yyyww","yyyyw","yyyyy","xyyyy","xxyyy","xxxyy","xxxxy","xxxxx","gxxxx","ggxxx","gggxx","ggggx","ggggg"]]
