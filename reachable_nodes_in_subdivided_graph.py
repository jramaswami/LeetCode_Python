...
class Solution:
    ...
    def reachableNodes(self, edges, max_moves, node_count):
        ...
        def dfs(node, moves, level):
            ...
            for neighbor in adj[node]:
                ...
                if moves > neighbor.subnodes:
                    ...
                    dfs(neighbor.node_id, moves - neighbor.subnodes - 1, level+1)
                else:
        ...
        for u, v in combinations(range(node_count), 2):
            key = (min(u, v), max(u, v))
...
def test_4():
    """WA"""
    # I needed to check all possible combinations of nodes, not just those
    # that are reached b/c ther may be reached subnodes on the way to an
    # unreached node.
