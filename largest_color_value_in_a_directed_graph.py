    ...
    def largestPathValue(self, colors: str, edges: List[List[int]]) -> int:
        ...
        def has_cycle(u):
            ...
            for v in adj[u]:
                if visited[v] == GRAY:
                    return True
                if visited[v] == WHITE and has_cycle(v):
                    return True
            visited[u] = BLACK
            return False

        for root in range(N):
            if visited[root] == WHITE and has_cycle(root):
                return -1
        ...
        def compute(color):
            ...
            def get_longest(u):
                ...
                result = current
                for v in adj[u]:
                    result = max(result, current + get_longest(v))
                has_cache[u] = True
                cache[u] = result
                return cache[u]
            ...
            result = 0
            for u in range(N):
                result = max(result, get_longest(u))
            return result

        # Acyclic graph!
        soln = 0
        for c in set(colors):
            soln = max(soln, compute(c))
        return soln
